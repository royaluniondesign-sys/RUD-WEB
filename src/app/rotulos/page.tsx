  ...
  return (
    <main style={{ background: '#0A0908' }}>
      ...
      <section style={{ position: 'relative', height: '100vh', minHeight: 640, overflow: 'hidden', display: 'flex', alignItems: 'flex-end' }}>
        ...
        <div className="container-custom" style={{ position: 'relative', zIndex: 2, paddingBottom: 'clamp(3rem,8vw,6rem)' }}>
          <div style={{ marginBottom: '1.5rem' }}>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 16px', borderRadius: 2, background: 'rgba(217,119,6,0.15)', border: '1px solid rgba(217,119,6,0.4)' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', marginBottom: '3rem' }}>
            <Link href="/contact?servicio=rotulos"
              style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '0.875rem 1.75rem', background: '#f8f8f8', color: '#171717', borderRadius: 2, fontWeight: 600, fontSize: 14, textDecoration: 'none' }}>
              Pedir presupuesto gratis →
            </Link>
            <a href="#tipos"
              style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '0.875rem 1.75rem', border: '1px solid rgba(255,255,255,0.25)', color: 'rgba(255,255,255,0.8)', borderRadius: 2, fontSize: 14, textDecoration: 'none' }}>
              Ver tipos de rótulo ↓
            </a>
          </div>
        ...
        <div className="container-custom">
                ...
                <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: 'clamp(1.5rem,4vw,2.5rem)' }}>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: '1rem' }}>
                    {['Neón LED', 'Neón de vidrio', 'Cualquier forma', 'Interior y exterior'].map(t => (
                      <span key={t} style={{ padding: '3px 10px', borderRadius: 2, fontSize: 11, fontWeight: 500, background: 'rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.7)', border: '1px solid rgba(255,255,255,0.15)' }}>{t}</span>
                    ))}
                  </div>
                  ...
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, marginTop: '1.25rem', padding: '0.625rem 1.25rem', background: 'rgba(255,255,255,0.12)', borderRadius: 2, fontSize: 13, fontWeight: 600, color: 'white', border: '1px solid rgba(255,255,255,0.2)' }}>
                    Presupuesto neón →
                  </div>
                </div>
              </div>
            ...
            {[
              ...
              <ScrollReveal key={item.title}>
                <Link href={item.href} style={{ display: 'block', textDecoration: 'none' }} className="group">
                  <div style={{ borderRadius: 12, overflow: 'hidden', position: 'relative', height: 300 }} className="card-lift">
                    ...
                    {item.tag && (
                      <div style={{ position: 'absolute', top: 16, left: 16 }}>
                        <span style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.18em', color: '#D97706', padding: '4px 12px', borderRadius: 2, background: 'rgba(217,119,6,0.2)', border: '1px solid rgba(217,119,6,0.35)' }}>{item.tag}</span>
                      </div>
                    )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: '1rem' }}>
            {[
              ...
              <ScrollReveal key={item.title}>
                <Link href={item.href} style={{ display: 'block', textDecoration: 'none' }} className="group">
                  <div style={{ borderRadius: 12, overflow: 'hidden', position: 'relative', height: 250 }} className="card-lift">
                    <img src={item.img} alt={item.alt} style={{ width: '100%', height: '100%', objectFit: 'cover', transition: 'transform 0.7s' }} />
      ...
      <section style={{ background: '#0D0D0D', padding: 'clamp(4rem,8vw,7rem) 0' }}>
        <div className="container-custom">
          <ScrollReveal>
            <p style={{ fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.18em', color: '#444', marginBottom: 12 }}>Cómo trabajamos</p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 1, background: '#1A1A1A', borderRadius: 12, overflow: 'hidden' }}>
            {PROCESO.map((s, i) => (
              <ScrollReveal key={s.n} delay={i * 70}>
                <div style={{ background: '#0D0D0D', padding: '2.5rem 2rem' }}>
                  <div style={{ color: '#555', marginBottom: '1.25rem' }}>{s.icon}</div>
                  <p style={{ fontSize: 10, fontFamily: 'monospace', color: '#2A2A2A', marginBottom: '0.75rem', letterSpacing: '.1em' }}>{s.n}</p>
        ...
        <div className="container-custom">
          <ScrollReveal>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', justifyContent: 'center' }}>
              <Link href="/contact?servicio=rotulos"
                style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '1rem 2rem', background: '#f8f8f8', color: '#171717', borderRadius: 2, fontWeight: 600, fontSize: 15, textDecoration: 'none' }}>
                Pedir presupuesto →
              </Link>
              <a href="tel:+34645593227"
                style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '1rem 2rem', border: '1px solid rgba(255,255,255,0.15)', color: 'rgba(255,255,255,0.65)', borderRadius: 2, fontSize: 14, textDecoration: 'none' }}>
                645 59 32 27
              </a>
            </div>
          </ScrollReveal>
